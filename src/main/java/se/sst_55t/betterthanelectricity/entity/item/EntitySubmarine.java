package se.sst_55t.betterthanelectricity.entity.item;
...
public class EntitySubmarine extends Entity {
    ...
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.<Integer>createKey(EntitySubmarine.class, DataSerializers.VARINT);
    ...
    private boolean sprintInputDown;
    ...
    protected void entityInit() {
        ...
        this.dataManager.register(BOAT_TYPE, Integer.valueOf(EntitySubmarine.Type.OAK.ordinal()));
    ...
    public void applyEntityCollision(Entity entityIn) {
        if (entityIn instanceof EntitySubmarine) {
    ...
    private EntitySubmarine.Status getBoatStatus() {
        EntitySubmarine.Status entityboat$status = this.getUnderwaterStatus();
                    ...
                    for (int i2 = i1; i2 < j1; ++i2) {
                        ...
                        if (iblockstate.getMaterial() == Material.WATER && d0 < (double) BlockLiquid.getLiquidHeight(iblockstate, this.world, blockpos$pooledmutableblockpos)) {
                            if (((Integer) iblockstate.getValue(BlockLiquid.LEVEL)).intValue() != 0) {
                                EntitySubmarine.Status entityboat$status = EntitySubmarine.Status.UNDER_FLOWING_WATER;
                                return entityboat$status;
                            }
    ...
    private void updateMotion() {
        ...
        if (this.previousStatus == EntitySubmarine.Status.IN_AIR && this.status != EntitySubmarine.Status.IN_AIR && this.status != EntitySubmarine.Status.ON_LAND) {
            ...
            if (this.status == EntitySubmarine.Status.IN_WATER) {
    ...
    private void controlBoat() {
        if (this.isBeingRidden()) {
            ...
            if(this.sprintInputDown && (this.status == Status.IN_WATER ||this.status == Status.UNDER_WATER || this.status == Status.UNDER_FLOWING_WATER))
            {
            ...
            if(this.jumpInputDown && (this.status == Status.UNDER_WATER || this.status == Status.UNDER_FLOWING_WATER))
            {
                if(this.motionY < 0.2F) {
                    this.motionY += 0.05F;
                }
    ...
    public void updatePassenger(Entity passenger) {
        if (this.isPassenger(passenger)) {
            ...
            if (passenger instanceof EntityPlayer) {
                ((EntityPlayer) passenger).addPotionEffect(new PotionEffect(MobEffects.WATER_BREATHING, 60));
            }
    ...
    protected void readEntityFromNBT(NBTTagCompound compound) {
        if (compound.hasKey("Type", 8)) {
            this.setBoatType(EntitySubmarine.Type.getTypeFromString(compound.getString("Type")));
        }
    }
    ...
    public void setBoatType(EntitySubmarine.Type boatType) {
        this.dataManager.set(BOAT_TYPE, Integer.valueOf(boatType.ordinal()));
    }

    public EntitySubmarine.Type getBoatType() {
        return EntitySubmarine.Type.byId(((Integer) this.dataManager.get(BOAT_TYPE)).intValue());
    }
    ...
    public static enum Type
    {
         ...
         */
        public static EntitySubmarine.Type byId(int id)
        {
        ...
        public static EntitySubmarine.Type getTypeFromString(String nameIn)
        {
