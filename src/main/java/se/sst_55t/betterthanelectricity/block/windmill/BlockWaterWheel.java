 ...
 */
public class BlockWaterWheel extends BlockTileEntity
{
    public static final PropertyDirection FACING = PropertyDirection.create("facing", EnumFacing.Plane.HORIZONTAL);

    public BlockWaterWheel(Material material, String name)
    {
        super(material, name);
    }
    ...
    {
        if (!this.canBlockStay(worldIn, pos, worldIn.getBlockState(pos).getValue(BlockWaterWheel.FACING) ))
        {
            worldIn.destroyBlock(pos, true);
        }
    }

    public boolean isWaterOrAir(Block block)
    {
    ...
    public boolean canBlockStay(World worldIn, BlockPos pos, EnumFacing facing)
    {
        ...
        switch(facing) {
            case NORTH:
                ...
                {
                    if(!(   isWaterOrAir(worldIn.getBlockState(pos.down()).getBlock()) &&
                            isWaterOrAir(worldIn.getBlockState(pos.east().down()).getBlock()) &&
                            isWaterOrAir(worldIn.getBlockState(pos.west().down()).getBlock())))
                    {
                        return false;
                    }
                    return false;
            ...
            case EAST:
                ...
                {
                    if(!(   isWaterOrAir(worldIn.getBlockState(pos.down()).getBlock()) &&
                            isWaterOrAir(worldIn.getBlockState(pos.north().down()).getBlock()) &&
                            isWaterOrAir(worldIn.getBlockState(pos.south().down()).getBlock())))
                    {
                        return false;
                    }
                    return false;
    ...
    @Override
    public Class<TileEntityWaterWheel> getTileEntityClass()
    {
        return TileEntityWaterWheel.class;
    }
